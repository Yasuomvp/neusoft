package com.neu.mybatisplus.controller;



import com.baomidou.mybatisplus.core.conditions.query.QueryWrapper;
import com.baomidou.mybatisplus.extension.api.ApiController;
import com.baomidou.mybatisplus.extension.api.R;
import com.baomidou.mybatisplus.extension.plugins.pagination.Page;
import com.neu.mybatisplus.entity.Car;
import com.neu.mybatisplus.entity.User;
import com.neu.mybatisplus.service.CarService;
import org.springframework.web.bind.annotation.*;

import javax.annotation.Resource;
import java.io.Serializable;
import java.util.List;

/**
 * (Car)表控制层
 *
 * @author makejava
 * @since 2022-07-06 12:05:05
 */
@RestController
@RequestMapping("car")
public class CarController extends ApiController {
    /**
     * 服务对象
     */
    @Resource
    private CarService carService;


    /**
     * 分页查询所有数据
     *
     * @param page 分页对象
     * @param car 查询实体
     * @return 所有数据
     */
    @GetMapping
    public R selectAll(Page<Car> page, Car car) {
        return success(this.carService.page(page, new QueryWrapper<>(car)));
    }

    /**
     * 通过主键查询单条数据
     *
     * @param id 主键
     * @return 单条数据
     */
    @GetMapping("{id}")
    public R selectOne(@PathVariable Serializable id) {
        return success(this.carService.getById(id));
    }

    /**
     * 新增数据
     *
     * @param car 实体对象
     * @return 新增结果
     */
    @PostMapping
    public R insert(@RequestBody Car car) {
        return success(this.carService.save(car));
    }

    /**
     * 修改数据
     *
     * @param car 实体对象
     * @return 修改结果
     */
    @PutMapping
    public R update(@RequestBody Car car) {
        return success(this.carService.updateById(car));
    }

    /**
     * 删除数据
     *
     * @param idList 主键结合
     * @return 删除结果
     */
    @DeleteMapping
    public R delete(@RequestParam("idList") List<Long> idList) {
        return success(this.carService.removeByIds(idList));
    }
}

